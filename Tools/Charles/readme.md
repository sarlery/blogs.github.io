...
## 请求与响应
在访问一个 web 站点（包括手机上的软件，都是依托 HTTP 协议的）时，你会发送一个网络请求，用来请求特定站点上的资源，服务器接收到你的请求时，就会做出相应，然后把你想要的资源发送到客户端让你阅览。  
...
### 设置成系统代理
不将 Charles 设置成系统代理，Charles 是截取不到网络封包的。设置很简单：来到菜单栏 --> Wroxy --> 点击选择“Windows Proxy”。设置完后，访问站点时，Charles 就会捕获封包。

### 安装证书
安装证书是为了让浏览器等软件信任 Charles 软件的数据拦截。因为当你访问一个站点时你自己是受信任的（因为那是你本人在上网），而使用 Charles 进行数据抓取时，浏览器会认为 Charles 是不受信任的第三方，可能会泄漏隐私，就不会让你访问该站点。没有证书在 Charles 中是不能查看封包中的内容的。  
...
你也可以使用底部的 filter（过滤）功能，将想要查看的内容过滤出来，比如我想看访问 淘宝网 的网络请求信息，就可以在 filter 中输入 `taobao` 字段过滤出特定的网址。  
