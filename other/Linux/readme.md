...
1. `mount` 命令（需要 root 权限）：查询系统中已经挂载的设备；  
2. `mount -a` （需要 root 权限）：依据配置文件（/etc/fstab）的内容，自动挂载；  
3. `mount [-t 文件系统] [-o 特殊选项] 设备文件名 挂载点`   
   选项：  
   `-t` 文件系统，加入文件系统类型，可以是 `ext3` `ext4` `iso9660`（挂载的是光盘）等文件系统；  
   
   `-o` 特殊选项，可以指定额外的挂在选项；  

    ![-o 特殊选项](img/-o特殊选项.png)  

    > 最好不要随意修改特殊选项，可能会让系统出现问题。在 Linux 中挂载光盘或者 U 盘，用完后应先卸载才能弹出光盘。（应该说所有的外来设备用完后都应该先卸载）  

4. 与 `mount` 挂在命令对应的是卸载命令 —— `unmount`  
   格式：`unmout 设备挂载点（就是设备所在目录）`  
...
> 除此之外 `who` 命令也可以查看用户（查看当前用户信息）。  

2. `last` 命令：查询当前登录和过去登录的用户信息。  
last 命令默认读取 `/var/log/wtmp` 的文件数据。  
命令输出：  
   - 用户名
   - 登录终端
   - 登录 IP
