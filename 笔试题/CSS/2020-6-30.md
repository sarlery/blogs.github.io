...
圣杯布局（Holy Grail） 从上到下由页头、内容和页脚组成，内容由左、中、右三列组成，其中左右两列的宽度固定，中间一列自适应。如下图所示。  

![圣杯布局](img/holy-grail.png)  

假设左右宽度是 100px，header 和 footer 的高度是 60px。
...
这中实现被称为“圣杯布局”。除了上面的方式之外，使用浮动 + `calc` 也可以做到中间部分自适应。HTML 骨架也不需要做调整。代码如下：  
