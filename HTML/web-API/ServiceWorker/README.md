...
4. 运行在与页面的 JavaScript 主线程独立的线程上；  
   ...
   使用其API大致遵循以下过程：  
5. 注册 —— 只有先注册了，才可以使用其功能。通过调用`serviceWorker.register`来注册。注册完之后，service worker就会被下载到客户端并尝试安装或激活。  
6. 下载 —— 注册之后，访问带有 service-worker 功能的网址时，该网站就会下载 service-worker 程序文件；  
7. 安装 —— 已在完之后会自动在后台安装；  
8. 激活 —— 第一次在页面或网站遇到service worker，如果下载的文件是新的，安装就会尝试进行，安装之后激活；如果现有service worker已启用，新版本会在后台安装，但不会被激活，这个时序称为worker in waiting。直到所有已加载的页面不再使用旧的service worker才会激活新的service worker。  
...
上面只是简单的介绍了一下 service-worker 的工作流程。可以参考MDN上非常详细的一个案例 —— **[Service workers demo](https://developer.mozilla.org/zh-CN/docs/Web/API/Service_Worker_API/Using_Service_Workers)**
